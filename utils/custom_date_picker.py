...
def popup_get_date(
    ...
    if day_abbreviations is None or len(day_abbreviations) != 7:
        fwday = sg.calendar.SUNDAY
        try:
            if locale is not None:
                _cal = sg.calendar.LocaleTextCalendar(fwday, locale)
            else:
                _cal = sg.calendar.TextCalendar(fwday)
    ...
    mon_names = (
        month_names
        if month_names is not None and len(month_names) == 12
        else [sg.calendar.month_name[i] for i in range(1, 13)]
    )
    ...
    if not close_when_chosen:
        layout += [
            [
                sg.Button("Ok", border_width=0, font="TkFixedFont 12"),
                sg.Button("Cancel", border_width=0, font="TkFixedFont 12"),
            ]
        ]
